...
export default function CompanyJobDetailPage() {
  ...
  const renderActionButtons = (app, phase) => {
    const buttonClass =
      "text-sm px-3 py-1.5 rounded-lg font-medium transition-all duration-200 hover:shadow-md transform hover:scale-105";
  ...
  const renderApplicantCard = (app, phase) => {
    const locationStr =
      app.user.location?.kabupaten?.name && app.user.location?.provinsi?.name
        ? `${app.user.location.kabupaten.name}, ${app.user.location.provinsi.name}`
        : "Remote / Not specified";

    const about = app.user.aboutMe?.trim() || "";
    const isExpanded = !!expandedAbout[app.user._id];
    const shortAbout = about.length > 220 ? about.slice(0, 220) + "…" : about;
      ...
      >
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              {app.user.profilePicture ? (
                <img
                  ...
                  className="w-12 h-12 rounded-full object-cover border border-gray"
                />
              ) : (
              ...
              <div>
                ...
                <p className="text-xs text-gray mt-0.5">{locationStr}</p>
              </div>

              <span
                className={`ml-auto px-3 py-1 rounded-full text-xs font-medium border ${getStatusBadgeColor(
                  app.status
                )}`}
              >
            ...
            {/* About Me */}
            <div className="mb-3">
              <p className="text-sm font-medium text-gray mb-1">About</p>
              {about ? (
                <>
                  <p className="text-sm text-color whitespace-pre-wrap">
                    {isExpanded ? about : shortAbout}
                  </p>
                  {about.length > 220 && (
                    <button
                      onClick={() =>
                        setExpandedAbout((prev) => ({
                          ...prev,
                          [app.user._id]: !prev[app.user._id],
                        }))
                      }
                ...
                <p className="text-sm text-gray italic">No bio provided.</p>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {app.user.curriculumVitae ? (
                ...
                <span className="inline-flex items-center gap-1 border border-gray text-gray bg-color-1 text-sm px-3 py-1.5 rounded-lg">
          ...
          {/* Actions */}
          <div className="flex flex-col gap-2">{renderActionButtons(app, phase)}</div>
        </div>
      </div>
    );
  };
      ...
      <div className="mt-4">
        {users.length === 0 ? (
          ...
          <>
            ...
            <div className="space-y-4">{users.map((app) => renderApplicantCard(app, phase))}</div>
          </>
        )}
      </div>
    );
  ...
  return (
    <div className="bg-color-1">
      <div className="p-6 max-w-6xl mx-auto bg-color-1 min-h-screen">
                ...
                <div className="flex items-center gap-4 text-sm text-gray">
                  ...
                  {job.salaryRange && (
                    <span className="flex items-center gap-1 text-green-600">
                      ...
                      {job.salaryRange.max?.toLocaleString()} {job.salaryRange.currency || "IDR"}
                    </span>
                  )}
                </div>
              </div>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`relative flex items-center gap-2 px-6 py-4 font-medium transition-all duration-300 whitespace-nowrap group ${
                  activeTab === tab.key ? "text-primary bg-primary-20" : "text-gray hover:text-primary hover:bg-color-1"
                }`}
              >
                ...
                {tab.count > 0 && (
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${
                      activeTab === tab.key ? "bg-primary text-white" : "bg-gray-200 text-gray-700"
                    }`}
                  >
                    {tab.count}
                  </span>
        ...
        {selectedReview && (
          <BaseModal isOpen={true} onClose={() => setSelectedReview(null)} title="Write a Review">
            <ReviewForm
          ...
          >
            <div className="space-y-6">
              <div className="p-4 bg-primary-20 rounded-lg border border-primary">
                ...
                <div className="text-sm text-gray">{job?.title}</div>
              </div>

              <p className="text-sm text-gray">
                Are you sure you want to mark this application as{" "}
                <span className="font-semibold">ended</span>? This should be used once the work is
                completed.
              </p>
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                ...
                className={`text-3xl transition-all duration-200 hover:scale-110 ${
                  star <= (hoveredRating || rating) ? "text-yellow-400" : "text-gray-300 hover:text-yellow-300"
                }`}
              >
                ★
              </button>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray">
          ...
          <button type="submit" className="btn-primary text-white font-medium shadow-md hover:shadow-lg transition-all duration-200">
            Submit Review
          </button>
        </div>
      </form>
