...
export default function LoginForm({ role, title, compact = false }) {
  ...
  return (
    <div className={`${compact ? "" : "flex items-center justify-center"} bg-background text-text px-4`}>
      <Toaster position="top-center" />
      <form
        onSubmit={handleLogin}
        className={`bg-color-2 shadow-xl rounded-xl w-full ${compact ? "p-4" : "p-8 max-w-md"} space-y-6`}
      >
        <h2 className={`text-center font-bold ${compact ? "text-xl" : "text-3xl"}`}>
        ...
        {!compact && (
          <div className="text-center text-sm flex justify-center items-center gap-2">
            <a href={`/auth/${role}/register`} className="text-sm text-primary hover:underline">
              Register
            </a>
            <span className="text-color">or</span>
            <a href={`/forgot-password?role=${role}`} className="text-sm text-primary hover:underline">
              Forgot password?
            </a>
          </div>
        )}
