...
export default function ProfileReminderModal() {
  ...
  const [resending, setResending] = useState(false); // ← add this
  ...
  const resendEmail = async () => {
    try {
      setResending(true);
      const tId = toast.loading("Resending verification email…");
      const token = localStorage.getItem("unicru-token");

      await axiosInstance.post("/auth/reverify", { token });

      toast.success("Verification email sent!", { id: tId });
    } catch (err) {
      const msg =
    ...
    <BaseModal isOpen={show} onClose={dismiss} title="Complete Your Profile">
              ...
              {isRecentlyCreated() ? (
                ...
                <span>
                  ...
                  <button
                    onClick={resendEmail}
                    disabled={resending}
                    className="ml-2 text-blue-600 underline font-bold disabled:opacity-60"
                  >
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="flex gap-2">
          <button
            onClick={dismiss}
            className="text-color border-1 bolder-black px-2 rounded-sm"
          >
            I'll do it later
          </button>
